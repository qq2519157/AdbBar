...
//! status item own it.
//!
//! `tray-icon` (≤0.24, the range `tauri` 2.x pins) attaches the tray's `NSMenu`
//! to the `NSStatusItem` once, at build time, and leaves it there. On macOS 27 a
//! status item that owns an `NSMenu` stops forwarding clicks to its view at all
//! (TN3212 gesture recognizers; tauri-apps/tray-icon#355), so
//! `TrayIconEvent::Click` never fires and `show_menu_on_left_click(false)` is
//! ignored — macOS just shows the menu, which makes the left-click popover
//! unreachable.
//!
//! Upstream fixed this in tray-icon 0.25.1 by attaching the menu to the status
//! item only while it is being presented, but `tauri` 2.x still requires
//! `tray-icon = "0.24"`, so the fix cannot be pulled in yet. Instead we apply
//! the same idea one level up: on affected systems the status item is built
//! with no menu — which is what restores the click events — and this module
//! presents the menu on right-click.
//!
//! Delete this module (and the branches in `lib.rs` / `main.rs`) once tauri
//! depends on tray-icon 0.25: the native path is better in every way.
...
const FORCE_ENV: &str = "ADBBAR_TRAY_DETACHED_MENU";
...
pub fn present<R: Runtime>(window: Window<R>, menu: &Menu<R>) {
    ...
    if let Err(err) = menu.popup(window) {
        eprintln!("could not present the tray menu: {err}");
    }
    highlight(false);
}

/// Anything but an explicit "off" turns the workaround on, so that a bare
/// `ADBBAR_TRAY_DETACHED_MENU=1` does the obvious thing.
